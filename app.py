...
def analyze_all():
    ...
    # 🔥 200銘柄をまとめて高速取得
    # =========================
    df_all = yf.download(tickers_list, period="6mo", group_by="ticker", threads=True)
...
with tab_jp:
    ...
    for idx, row in df_jp.head(5).iterrows():
        ...
        st.write(f"RSI：{row['RSI']} / MACD：{row['MACD']} / Signal：{row['Signal']}")
        st.write("---")
    ...
    if len(df_jp) > 0:
        ...
        ax.set_title(f"{selected_name}（{ticker}）")
        ax.legend()
        ax.grid(True)
        st.pyplot(fig)
